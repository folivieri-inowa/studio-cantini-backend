...
function getDefaultMetadata() {
  return {
    ...
    identification: { document_number: null, issue_date: null, expiry_date: null, issuing_authority: null },
    parties: {
    ...
    financial: { amounts: [], payment: { due_date: null, method: null, reference: null, iban: null } },
    content: { summary: '', key_facts: [], dates: [], locations: [] },
    custom_fields: {},
    services: [],
    products: [],
    keywords: [],
...
// Recupera e processa SW-2024-01.pdf
const client = await pool.connect();
try {
  const docs = await client.query("SELECT id, extracted_text FROM archive_documents WHERE original_filename = 'SW-2024-01.pdf'");
  ...
  console.log('📄 Estrazione metadata per:', doc.id);
  console.log('\nTesto:');
  console.log('='.repeat(50));
  console.log(doc.extracted_text);
  console.log('='.repeat(50));

  console.log('\n🤖 Chiamata LLM...\n');
  const metadata = await extractMetadataWithLLM(doc.extracted_text);
  ...
  // Salva nel DB
  await client.query(
    'UPDATE archive_documents SET extracted_metadata = $1, processing_status = $2 WHERE id = $3',
    [JSON.stringify(metadata), 'completed', doc.id]
  );
  console.log('\n💾 Metadata salvati e stato aggiornato a completed');
