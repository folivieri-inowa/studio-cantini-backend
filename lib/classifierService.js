...
async function findHistoricalMatch(transaction, db, pg) {
  ...
  const query = `
    WITH scored_feedback AS (
      SELECT 
        ...
        -- Recency score (più recente = meglio)
        1.0 - LEAST(1.0, EXTRACT(EPOCH FROM (NOW() - cf.created_at)) / (86400.0 * 180.0)) as recency_score,
        
        -- Frequency score (quante volte questa combo è stata usata)
        (
          SELECT COUNT(*)::float / 20.0 
      ...
      WHERE cf.db = $1
        AND similarity(cf.original_description, $2) > 0.75
    ...
    SELECT 
      *,
      ...
      (text_similarity * $6 + 
       amount_proximity * $7 + 
       recency_score * $8 +
       LEAST(1.0, frequency_score) * $9) as combined_score
    FROM scored_feedback
    WHERE text_similarity >= $10
      AND amount_proximity >= $11
  ...
  const result = await pg.query(query, [
    ...
    weights.recency,
    weights.frequency,
    config.thresholds.textSimilarityMin,
  ...
  const confidence = Math.min(95, Math.round(match.combined_score * 100));
  
  if (confidence < config.thresholds.exactConfidenceMin) {
    return null;
  }
  
  console.log(`[STAGE 2: Exact] Match storico con confidence ${confidence}% per transazione ${transaction.id}`);
  ...
  return {
    ...
    confidence,
    method: 'exact',
    reasoning: `Match storico: "${match.matched_description}" (${formatCurrency(match.matched_amount)}, ${Math.round(match.text_similarity * 100)}% similarità)`,
    debug: {
      text_similarity: Math.round(match.text_similarity * 100),
      amount_proximity: Math.round(match.amount_proximity * 100),
      recency_score: Math.round(match.recency_score * 100),
      frequency_score: Math.round((match.frequency_score || 0) * 100),
      combined_score: Math.round(match.combined_score * 100),
    }
  };
}
