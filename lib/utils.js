...
function getCellValue(cell) {
  ...
  if (cell.t === 'n') {
    ...
    if (cell.w && /\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}/.test(cell.w)) {
      // Converti il numero seriale Excel in data ISO (DD/MM/YYYY) per evitare
      // ambiguità locale (cell.w può essere M/D/YY su sistemi en-US)
...
export function parseDate(dateString) {
  ...
  // Prova formato americano con anno a 2 cifre (es. 1/13/26 da ExcelJS)
  date = DateTime.fromFormat(dateString, 'M/d/yy');
  if (date.isValid) return date.toFormat('yyyy-MM-dd');
