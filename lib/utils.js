...
export function detectPaymentMethod(description) {
  ...
  if (/\bpos\b/.test(lowerDesc)) return "POS"; // Cerca "POS" come parola intera
  if (/\bdisposizione\b|\bbonif\b|\bbonifico\b|\bgiroconto\b/.test(lowerDesc)) return "Bonifico";
  if (/\bf24\b/.test(lowerDesc)) return "F24";
  if (/\bCbill\b/.test(lowerDesc)) return "Cbill";
  if (/\bpaypal\b/.test(lowerDesc)) return "PayPal";
  if (/\bsepa\b/.test(lowerDesc)) return "Addebito Diretto SEPA";
  if (/\bnexi\b/.test(lowerDesc)) return "Carte di Credito";
