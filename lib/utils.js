...
export async function ConvertExcelToJson(fileBuffer) {
  ...
  // Usa sempre il primo foglio disponibile
  if (workbook.SheetNames.length === 0) {
    console.error('❌ Nessun foglio trovato nel file');
    return [];
  }
  const sheetName = workbook.SheetNames[0];
  console.log('📋 Foglio utilizzato:', sheetName);
...
function processWorksheet(worksheet) {
  ...
  if (rawRows.length > 0) {
    ...
    // Detect if first row is a header (text) or data (date/number)
    const firstAisDate = firstRow.A && (
      (typeof firstRow.A === 'string' && /\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}/.test(firstRow.A)) ||
      (typeof firstRow.A === 'number' && firstRow.A > 10000) // Excel date serial
    );
    const firstRowIsData = firstAisDate;
    ...
    if (!firstRow.A && firstRow.B && typeof firstRow.B === 'string' &&
      ...
      format = 'alternate';
    } else if (!firstRowIsData) {
      // Prima riga sembra un header (testo) — rilevamento tramite intestazioni
      const headerC = String(firstRow.C || '').toLowerCase();
      const headerD = String(firstRow.D || '').toLowerCase();
      const hasDareAvereHeaders = headerC.includes('dare') || headerD.includes('avere') || headerD.includes('credito');

      const sampleRows = rawRows.filter(r => r.A);
      const hasAnyD = sampleRows.some(r => r.D !== null && r.D !== '' && r.D !== 0);
      if (hasDareAvereHeaders || (hasAnyD && sampleRows.length > 0)) {
      ...
      // Prima riga è dati (no header) — default a standard per file 4-colonne
      console.log('📋 File senza intestazioni → formato standard (A=data, B=descrizione, C=dare, D=avere)');
    }
  }
  ...
  for (const raw of rawRows) {
    ...
    if (dateField) {
      ...
      if (format === 'alternate' || format === 'single') {
        ...
        // Forza il segno: dare sempre negativo, avere sempre positivo
        const rawDare = raw.C;
        const rawAvere = raw.D;
        const parsedDare = rawDare !== null && rawDare !== '' ? parseAmount(rawDare) : null;
        const parsedAvere = rawAvere !== null && rawAvere !== '' ? parseAmount(rawAvere) : null;
        if (parsedDare !== null && parsedDare !== 0) negativeAmount = -Math.abs(parsedDare);
        if (parsedAvere !== null && parsedAvere !== 0) positiveAmount = Math.abs(parsedAvere);

        // DEBUG: log prime 5 righe con importo
        if (debugCount < 5 && (negativeAmount !== null || positiveAmount !== null)) {
          console.log(`🔍 [DEBUG riga ${debugCount}] raw.C=${JSON.stringify(rawDare)} (type=${typeof rawDare}), raw.D=${JSON.stringify(rawAvere)} (type=${typeof rawAvere})`);
          console.log(`   parsedDare=${parsedDare}, parsedAvere=${parsedAvere} → negativeAmount=${negativeAmount}, positiveAmount=${positiveAmount}`);
          debugCount++;
        }
      }
...
export function parseDate(dateString) {
  ...
  // Rimuovi spazi normali e non-breaking spaces (U+00A0) da entrambi i lati
