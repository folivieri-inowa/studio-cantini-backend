  ...
  for (const raw of rawRows) {
    ...
    if (dateField) {
      ...
      if (format === 'alternate' || format === 'single') {
        ...
        if (raw.C !== null && raw.C !== '') {
          const dare = parseAmount(raw.C);
          if (dare !== null && dare !== 0) negativeAmount = -Math.abs(dare);
        }
        if (raw.D !== null && raw.D !== '') {
          const avere = parseAmount(raw.D);
          if (avere !== null && avere !== 0) positiveAmount = Math.abs(avere);
        }
      }
